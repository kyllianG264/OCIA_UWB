...
from court_geometry import court_bounds
from cv_track_stable_ids import build_stable_frames as build_hungarian_kalman_cv_frames

try:
    from scene import CAMERA_X, CAMERA_Y
except ImportError:
    from estimation_2d.scene import CAMERA_X, CAMERA_Y


DEFAULT_CV_LOG_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "CV Logs", "positions_merged.csv")
DEFAULT_CV_CALIBRATION_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "CV Logs", "calibration.json")
DEFAULT_CV_VIDEO_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "CV Logs", "tracking_composite.mp4")
DEFAULT_CV_METADATA_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "CV Logs", "run_metadata.json")
...
def _load_cv_calibration(calibration_path):
    ...
    halves = []
    for name, bounds in terrain_bounds.items():
    ...
    if not halves:
        return None
    left = min(item["x_min"] for item in halves)
    right = max(item["x_max"] for item in halves)
    top = min(item["y_min"] for item in halves)
    bottom = max(item["y_max"] for item in halves)
    split_candidates = []
    for item in halves:
        split_candidates.extend([item["y_min"], item["y_max"]])
    split_y = sorted(split_candidates)[len(split_candidates) // 2]
    return {
...
class CvLogSource:
    def __init__(self, csv_path, calibration_path=None, player_id=None, video_path=None, expected_player_count=None):
        ...
        self.court_bounds = court_bounds(CAMERA_X, CAMERA_Y)
    ...
    def _map_position_to_scene(self, x_cv, y_cv):
        if not self.calibration:
            return x_cv, y_cv
        left_cv, right_cv, top_cv, bottom_cv = self.calibration["bounds"]
        left_scene, right_scene, top_scene, bottom_scene = self.court_bounds
        width_cv = max(right_cv - left_cv, 1.0)
        height_cv = max(bottom_cv - top_cv, 1.0)
        nx = min(1.0, max(0.0, (x_cv - left_cv) / width_cv))
        ny = min(1.0, max(0.0, (y_cv - top_cv) / height_cv))
        scene_x = right_scene - ny * (right_scene - left_scene)
        scene_y = top_scene + nx * (bottom_scene - top_scene)
