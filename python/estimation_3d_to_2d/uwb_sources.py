...
def project_3d_distance_to_2d(d3, dz_assumed):
    if d3 is None:
        return None
    value = d3 * d3 - dz_assumed * dz_assumed
...
def get_simulated_distances(tag_xy, tag_height, anchors, settings):
    ...
    for aid, anchor_xy in anchors.items():
        true_d = distance_3d_xy_z(tag_xy, tag_height, anchor_xy, settings["anchor_height_cm"])
        raw_3d[aid] = noisy_distance(true_d, settings)
    return {
        "raw_3d": raw_3d,
