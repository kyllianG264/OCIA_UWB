...
def main(argv=None):
    ...
    pygame.display.set_caption("UWB - Estimation 3D vers 2D")
    ...
    try:
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if heatmap_button_rect().collidepoint(event.pos):
                        ...
                        tag_real, _, _ = compute_real_tag_position(state["t"], state, 0.0)
            ...
            if source.uses_simulated_tag:
                tag_real, tag_height, jump_extra = compute_real_tag_position(state["t"], state, dt)
            else:
                tag_real = None
                tag_height = settings["tag_base_height_cm"]
                jump_extra = 0.0
            distance_packet = source.get_distances(tag_real, tag_height, active_anchors, settings)
            ...
            draw_scene(screen, (font, small_font), active_anchors, active_ids, state, solution, tag_real, tag_height, jump_extra, layout_name, active_anchor_count)
            pygame.display.flip()
    finally:
        source.close()
        pygame.quit()
