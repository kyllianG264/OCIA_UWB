...
_CORE_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "estimation_2d", "position_calcul.py")
_SPEC = importlib.util.spec_from_file_location("_estimation_2d_position_calcul", _CORE_PATH)
...
def update_position_solution(anchors, distance_packet, state, tag_real=None):
    result = _CORE.update_position_solution(anchors, distance_packet, state, tag_real)
    result["dist_raw_3d"] = distance_packet.get("raw_3d", {})
    return result
