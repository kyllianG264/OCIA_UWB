...
from scene import FOV_DEG, FPS, HEIGHT, TAG_BASE_Z_CM, WIDTH, adjust_anchor, adjust_setting, create_state, full_reset, get_anchor_layout, load_settings, refresh_runtime_state, save_settings, set_active_anchor_count
...
def main(argv=None):
    parse_args(argv)
    import pygame
    from display import OrbitCamera, clamp, draw_scene, heatmap_button_rect

    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))
    pygame.display.set_caption("UWB - Estimation 3D")
    ...
    while running:
        ...
        if solution["tag_est_smooth"] is not None:
            ...
            update_player_analytics(state["player_analytics"], state["t"], (estimated[0], estimated[1]), estimated[2], estimated_jump_extra, dt)
        state["trail_real"].append(tag_real)
        if solution["tag_est_smooth"] is not None:
            state["trail_est"].append(solution["tag_est_smooth"])
        draw_scene(screen, camera, focal, (font, small_font), active_anchors, active_ids, state, solution, tag_real, jump_extra, show_rays, show_trails, layout_name, active_anchor_count, build_control_items(state["settings"]))
        pygame.display.flip()
