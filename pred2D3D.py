...
# CONFIG HAUTEURS (cm)
# =========================
ANCHOR_HEIGHT_CM = 1500          # hauteur des Portenta
TAG_BASE_HEIGHT_CM = 20.0         # hauteur normale Stella
TAG_ASSUMED_HEIGHT_CM = 20.0      # hauteur supposée par le solveur 2D

# =========================
# SAUTS VERTICAUX
# =========================
JUMP_EXTRA_HEIGHT_CM = 100.0      # saut de ~1 m
AUTO_JUMP_PROB_PER_SEC = 0.35     # probabilité de départ d'un saut
...
def distance_3d_xy_z(p_xy, z1, a_xy, z2):
    dx = a_xy[0] - p_xy[0]
    dy = a_xy[1] - p_xy[1]
    dz = z2 - z1
...
def start_jump():
    global jump_active, jump_timer, jump_duration, jump_extra_height
    jump_active = True
    jump_timer = 0.0
    jump_duration = random.uniform(JUMP_DURATION_MIN, JUMP_DURATION_MAX)
    jump_extra_height = JUMP_EXTRA_HEIGHT_CM * random.uniform(0.90, 1.10)

def update_jump(dt):
    global jump_active, jump_timer
    ...
    # saut lisse : monte puis redescend
    return jump_extra_height * math.sin(math.pi * p)

def full_reset(anchor_count):
    global t, stats, dist_3d_smooth, tag_est_smooth
    global jump_active, jump_timer, jump_duration, jump_extra_height
    ...
    dist_3d_smooth = {aid: None for aid in active_anchors_tmp.keys()}
...
pygame.display.set_caption("UWB 3D -> projection 2D avec sauts de hauteur")
...
while running:
    ...
    if set(dist_3d_smooth.keys()) != set(active_ids):
        dist_3d_smooth = {aid: None for aid in active_ids}

    # =========================
    # TAG RÉEL SIMULÉ EN 2D
    # =========================
    ...
    # =========================
    # HAUTEUR RÉELLE DU TAG
    # =========================
    jump_extra = update_jump(dt)
    tag_height_real = TAG_BASE_HEIGHT_CM + jump_extra

    dist_3d_raw = {}
    dist_2d_projected_raw = {}
    dist_2d_projected_smooth = {}

    for aid, pos in active_anchors.items():
        anchor_height = ANCHOR_HEIGHT_CM

        # vraie distance UWB 3D
        d3_true = distance_3d_xy_z(tag_real, tag_height_real, pos, anchor_height)

        # mesure UWB bruitée
        d3_raw = noisy_distance(d3_true)
        dist_3d_raw[aid] = d3_raw

        # lissage de la mesure UWB 3D
        dist_3d_smooth[aid] = smooth_value(dist_3d_smooth[aid], d3_raw, ALPHA_DIST)

        # correction vers le plan 2D avec hauteur supposée fixe
        dz_assumed = anchor_height - TAG_ASSUMED_HEIGHT_CM

        d2_raw = project_3d_distance_to_2d(d3_raw, dz_assumed)
        d2_smooth = project_3d_distance_to_2d(dist_3d_smooth[aid], dz_assumed)

        dist_2d_projected_raw[aid] = d2_raw
        dist_2d_projected_smooth[aid] = d2_smooth
    ...
    if len(active_ids) >= 2:
        ...
        raw_intersections = circle_intersections(
            active_anchors[a1][0], active_anchors[a1][1], dist_2d_projected_raw[a1],
            active_anchors[a2][0], active_anchors[a2][1], dist_2d_projected_raw[a2]
        )

        smooth_intersections = circle_intersections(
            active_anchors[a1][0], active_anchors[a1][1], dist_2d_projected_smooth[a1],
            active_anchors[a2][0], active_anchors[a2][1], dist_2d_projected_smooth[a2]
        )

    tag_est_raw = estimate_position_least_squares(active_anchors, dist_2d_projected_raw)
    tag_est_from_smooth = estimate_position_least_squares(active_anchors, dist_2d_projected_smooth)
    ...
    # Cercles projetés 2D bruts transparents
    overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    for aid in active_ids:
        ax, ay = active_anchors[aid]
        sx, sy = world_to_screen(ax, ay)
        radius = max(1, int(dist_2d_projected_raw[aid] * SCALE))
        c = anchor_colors[aid]
        pygame.draw.circle(overlay, (c[0], c[1], c[2], 70), (sx, sy), radius, 2)
    screen.blit(overlay, (0, 0))

    # Cercles projetés 2D lissés
    for aid in active_ids:
        ax, ay = active_anchors[aid]
        sx, sy = world_to_screen(ax, ay)
        radius = max(1, int(dist_2d_projected_smooth[aid] * SCALE))
    ...
    draw_text(screen, "Tag réel (sol)", tx + 16, ty - 12, font, (255, 0, 0))

    # Halo si saut en cours
    if jump_active:
    ...
    # HUD
    hud_bg = pygame.Surface((760, 760), pygame.SRCALPHA)
    ...
    draw_text(screen, layout_name, 30, 58, small_font, (220, 220, 220))
    draw_text(screen, "Touches : 2..6 changer | R reset | ESPACE saut | ESC quitter", 30, 86, small_font, (220, 220, 220))
    draw_text(screen, "Mode : mesure UWB 3D -> reprojection 2D avec hauteur supposée fixe", 30, 114, small_font, (220, 220, 220))
    draw_text(screen, f"Tolérance précision : {PRECISION_TOLERANCE_CM:.0f} cm", 30, 142, small_font, (220, 220, 220))

    jump_color = (255, 120, 120) if jump_active else (180, 255, 180)
    draw_text(screen, f"Hauteur ancre              : {ANCHOR_HEIGHT_CM:.1f} cm", 30, 182, small_font, (220, 220, 220))
    draw_text(screen, f"Hauteur Stella réelle      : {tag_height_real:.1f} cm", 30, 208, small_font, jump_color)
    draw_text(screen, f"Hauteur Stella supposée    : {TAG_ASSUMED_HEIGHT_CM:.1f} cm", 30, 234, small_font, (220, 220, 220))
    draw_text(screen, f"Extra saut actuel          : {jump_extra:.1f} cm", 30, 260, small_font, jump_color)
    draw_text(screen, f"Saut en cours              : {'OUI' if jump_active else 'NON'}", 30, 286, small_font, jump_color)

    y0 = 326
    for aid in active_ids:
        draw_text(
            screen,
            f"A{aid}  3D brut={dist_3d_raw[aid]:7.1f} cm   2D proj brut={dist_2d_projected_raw[aid]:7.1f} cm   2D proj lissé={dist_2d_projected_smooth[aid]:7.1f} cm",
            30,
            y0,
            small_font,
            anchor_colors[aid]
    ...
    if active_anchor_count == 2:
        ...
        y0 += 28
        draw_text(screen, "Les cercles affichés sont les rayons projetés au sol.", 30, y0, small_font, (255, 180, 120))
        y0 += 28
        draw_text(screen, "Pas de précision moyenne fiable avec 2 ancres.", 30, y0, small_font, (255, 180, 120))
    else:
